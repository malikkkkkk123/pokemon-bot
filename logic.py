...
class Pokemon:
    ...
    def __init__(self, pokemon_trainer):
        ...
        self.power = random.randint(1, 40)
        self.hp = random.randint(40, 500)
    ...
    async def attack(self, enemy):
        ...
        if enemy.hp > self.power:
            enemy.hp -= self.power
            return f"Pertarungan @{self.pokemon_trainer} dengan @{enemy.pokemon_trainer} membuat nyawa @{enemy.pokemon_trainer} berkurang menjadi {enemy.hp}"
        else:
            enemy.hp = 0
    ...
    async def info(self):
        ...
        if not self.name:
            self.ability_name = await self.ability()
            self.name = await self.get_name()  # Retrieving a name if it has not yet been uploaded
        return f"The name of your Pokémon: {self.name}, the ability name of your pokemon: {self.ability_name}, and have {self.power} power, with {self.hp} hp."  # Returning the string with the Pokémon's name
...
class Fighter(Pokemon):
    ...
    async def feed(self, feed_interval = 20, hp_increase = 40 ):
        current_time = datetime.now()
        delta_time = timedelta(hours=feed_interval)  
        if (current_time - self.last_feed_time) > delta_time:
            self.hp += hp_increase
